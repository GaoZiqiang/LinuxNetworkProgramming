...
pthread_cond_t g_clientcond /*= PTHREAD_COND_INITIALIZER*/;
...
void prog_exit(int signo)
{
    ...
    ::pthread_cond_destroy(&g_clientcond);
...
void* accept_thread_func(void* arg)
{
    while (!g_bStop)
    {
        ...
        struct epoll_event e;
        memset(&e, 0, sizeof(e));
        e.events = EPOLLIN | EPOLLRDHUP | EPOLLET;// EPOLLET--边缘触发
        e.data.fd = newfd;
...
void* worker_thread_func(void* arg)
{
    while (!g_bStop)
    {
        ...
        while (g_listClients.empty())
            ::pthread_cond_wait(&g_clientcond, &g_clientmutex);
        ...
        while (true)
        {
            ...
            if (nRecv == -1)
            {
                if (errno == EWOULDBLOCK)
                    break;// ET触发--内核接收缓冲区中的数据已被读完--不能再读了--返回EWOULDBLOCK错误
                else// client端中断
                {
...
//        daemon_run();
    ...
    ::pthread_cond_init(&g_clientcond, NULL);
//    ::pthread_mutex_init(&g_mutex, NULL);
    ...
    while (!g_bStop)
    {
        ...
        int n = ::epoll_wait(g_epollfd, ev, 1024, 10);// epoll_wait()为单线程执行
        if (n == 0)
            continue;
        else if (n < 0)
        {
        ...
        for (int i = 0; i < m; ++i)
        {
            ...
            if (ev[i].data.fd == g_listenfd)
                ...
                //通知普通工作线程接收数据--一个线程只处理一个client的一次IO(事件)
            else
            {
                pthread_mutex_lock(&g_clientmutex);
                g_listClients.push_back(ev[i].data.fd);
                pthread_mutex_unlock(&g_clientmutex);
                pthread_cond_signal(&g_clientcond);
